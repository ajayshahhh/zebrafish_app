...
from flask import Flask, request, render_template, send_from_directory, abort, send_file, after_this_request
...
def process_video(video_file, model, start_time, end_time):
    ...
    for i in range(total_frames_to_process):
        ...
        frame_tensor = TF.to_tensor(processed_frame).unsqueeze(0).to(device)
        ...
        save_image(output_image, mask_filename, app.config['MASK_FOLDER'])
...
def cleanup_folders():
    # Using glob to find and remove all files in specified directories
    folders_to_cleanup = [app.config['UPLOAD_FOLDER'], app.config['MASK_FOLDER']]
    for folder in folders_to_cleanup:
        files = glob.glob(os.path.join(folder, '*'))  # Using glob to find all files in the folder
...
def download_results(video_id):
    ...
    # Move the masked images using glob
    if not os.path.exists(masked_images_folder):
        os.makedirs(masked_images_folder, exist_ok=True)
        
    for file in glob.glob(os.path.join(app.config['MASK_FOLDER'], f"{video_id}_*.png")):
        if 'annotated' not in file:
            shutil.copy(file, masked_images_folder)
            print(f"Copied {file} to {masked_images_folder}")

    # Move the annotated images using glob
    if not os.path.exists(annotated_images_folder):
        os.makedirs(annotated_images_folder, exist_ok=True)
        
    for file in glob.glob(os.path.join(app.config['MASK_FOLDER'], f"{video_id}_*annotated.png")):
        shutil.copy(file, annotated_images_folder)
        print(f"Copied {file} to {annotated_images_folder}")

    # Move the CSV file
    csv_file = os.path.join(app.config['MASK_FOLDER'], f"{video_id}_analysis.csv")
    if os.path.exists(csv_file):
        shutil.copy(csv_file, base_folder)
